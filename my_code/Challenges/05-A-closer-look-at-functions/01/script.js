...
const poll = {
  ...
  registerNewAnswer () {
    ...
    const optionRegistered = Number(prompt(`${this.question}\n ${this.options[0]} \n ${this.options[1]} \n ${this.options[2]} \n ${this.options[3]} \n Write option number: `));

    //1.2. Based on the input number, update the 'answers' array property.
    if (typeof optionRegistered === 'number' && optionRegistered >= 0 && optionRegistered <=3) {
      this.answers[optionRegistered]++;
  ...
  displayResults () {
    ...
    for(let i = 0; i<this.options.length; i++){
      pollResult = `${this.options[i].slice(3)}: ${this.answers[i]}`;
...
//2. Call this method whenever the user clicks the "Answerpoll" button.
const registerNewAnswerBind = poll.registerNewAnswer.bind(poll);

document.querySelector('.poll').addEventListener('click', registerNewAnswerBind);
